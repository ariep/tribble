...
editLabel autofocus i = fmap (either id id) <$> toggleModesDyn
...
showLabel l = elClass "span" "label label-default" $ text l 
...
autocomplete parentActive input = mdo
  ...
  (u, e) <- elDynAttr' "ul" attrs $
    fmap switchPromptlyDyn . (mapDyn leftmost =<<) $ simpleList list $ \ item ->
      dynEvent $ ffor item $ \ x -> mdo
        attrs <- mapDyn (("class" =:) . bool "" "active") =<< hovered li
        (li, l) <- elDynAttr' "li" attrs . link $ x
